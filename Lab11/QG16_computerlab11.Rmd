...
$p(data | \theta)$ is the likelihood and $p(data)$ is the probability of producing the given data (which is acting as a normalizing constant and generally is difficult to calculate). The expression can be simplified as:

$$p(\theta| data) \propto L(\theta | data ) p (\theta)$$

The objective of Bayesian inference is to estimate the distribution of $\theta$ given the data. However, as you can see it takes a very complicated form it is not an easy problem. This is why we use methods like the Markov Chain Monte Carlo (MCMC) to generate "samples" that will represent the posterior distribution. The Metropolis-Hastings alogrithm is a widely used algorithm for this purpose. 
...
+ You can generate point densities by using random $\theta$ values that "jump around", and accept the value (or keep as a sample) depending on how likely the density is compared to the previous $\theta$'s density.
...
proposal_func <- function(parameters){
    proposal.output <- rnorm(3,mean = parameters, sd = c(0.1,0.1,0.1)) # why do we need an abs for the sd?
